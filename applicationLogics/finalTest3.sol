	/* 
		1. 테스트 명 : item 비교 로직 추가.
			sendCurrentTx()를 6번 보냄.
			getDistributionChannelStackIndex()를 Promise.all로 getMappingLength()만큼 호출함.
			첫번째 인자가 true인 stack정보만 모은다.
			거래가 존재하는 stack정보를 담은 stackinfo의 횟수만큼 getTx()를 Promise.all로 호출함.
			stack들을 번호를 붙여 const distributionChannels = [];에 저장하게 한다.
		2. debugger-ui :
