contract Simple {
	...
	function showAllArrayArguments(address _a) public returns (uint _length){
		uint tempSum;
		uint i;
		while(i < distributionChannelTable[_a].length){
		    tempSum = tempSum + distributionChannelTable[_a][i].addressIndex;
		    i++;
		}
